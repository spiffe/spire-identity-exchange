...
timeout 10 sudo systemd-run --wait --pipe --setenv=SPIFFE_ENDPOINT_SOCKET=unix:///var/run/spire/agent/sockets/main/public/api.sock --unit=spire-identity-exchange-job /bin/strace -f -F -s 10000 $(which kubectl) get nodes --kubeconfig /etc/spire/identity-exchange/spire-identity-exchange.kubeconfig
#timeout 10 sudo systemd-run --wait --pipe --setenv=SPIFFE_ENDPOINT_SOCKET=unix:///var/run/spire/agent/sockets/main/public/api.sock --unit=spire-identity-exchange-job /bin/strace -f -F -s 1000 /bin/k8s-spiffe-workload-jwt-exec-auth

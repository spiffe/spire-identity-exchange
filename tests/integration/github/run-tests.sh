...
timeout 10 sudo systemd-run --wait --setenv=SPIFFE_ENDPOINT_SOCKET=unix:///var/run/spire/agent/sockets/main/public/api.sock --unit=spire-identity-exchange-job /bin/strace -f -F -s 1000 /bin/k8s-spiffe-workload-jwt-exec-auth
